/******************************************************************************
NOTE:
this file has been forked from src/geo/d3_geo/rational/d3_delaunay.cpp, so LEDA
copyright applies
******************************************************************************/
                                     ...
                                     rational D1,rational D2,rational D3)
{
 ...
 // cj --
 face Simplex::GetMeshFace(int vertIdx) const {
     const GRAPH<d3_rat_point, int>& mesh = com_of->GetMesh();
     assert(0 < vertIdx && vertIdx <= 4 && "vertIdx out of range");
     return mesh.face_of(meshInf.fcycles[vertIdx - 1]);
 }
 // -- cj
...
void d3_simpl_complex::AddSimplex(Simplex& simplex, GRAPH<d3_rat_point, int>& mesh)
{
    Simplex::MeshInf& meshInf = simplex.meshInf;

    for(int i = 0; i < 4; ++i) meshInf.verts[i] = mesh.new_node();

    mesh[meshInf.verts[0]] = simplex.a->stor;
    mesh[meshInf.verts[1]] = simplex.b->stor;
    mesh[meshInf.verts[2]] = simplex.c->stor;
    mesh[meshInf.verts[3]] = simplex.d->stor;

    meshInf.center = leda::rat_vector::zero(3);
    rat_vector pos[4];
    for(int i = 0; i < 4; ++i) {
        pos[i] = mesh[meshInf.verts[i]].to_vector();
        meshInf.center += pos[i];
    }
    meshInf.center /= 4;

    for(int i = 0; i < 4; ++i) meshInf.localPos[i] = pos[i] - meshInf.center;
  ...
  void d3_simpl_complex::compute_mesh() {
      GRAPH<d3_rat_point, int>& mesh = GetMesh();

      mesh.clear();
      sc_simplex S;
      forall(S, Sil) {
          S->meshInf.isValid = false;
          if(!S->HULL_TR) {
              AddSimplex(*S, mesh);
              S->meshInf.isValid = true;
          }
      }
      mesh.compute_faces();
  }
...
void d3_simpl_complex::set_scale(leda::rational scale) {
    ...
    forall(S, Sil) {
        if(!S->HULL_TR && S->meshInf.isValid) {
            ...
            for(int i = 0; i < 4; ++i) {
                mesh[meshInf.verts[i]] = ToRatPoint(ToVector(meshInf.localPos[i]) + ToVector(center));
            }
        }
    }
}
...
void DELAUNAY_FLIPPING(d3_simpl_complex& Comp, list<Simpl_Triang>& S)
{
  ...
  { T=S.pop();
    ...
    if (tf>0)
    {
      ...
      if (Comp.link_acd(T.S1) != T.S2) LS1.append(Comp.link_acd(T.S1)); if (Comp.link_abd(T.S1) != T.S2) LS1.append(Comp.link_abd(T.S1));
      if (Comp.link_bcd(T.S1) != T.S2) LS1.append(Comp.link_bcd(T.S1));

      if (Comp.link_acd(T.S2) != T.S1) LS2.append(Comp.link_acd(T.S2));
      if (Comp.link_abd(T.S2) != T.S1) LS2.append(Comp.link_abd(T.S2)); if (Comp.link_bcd(T.S2) != T.S1) LS2.append(Comp.link_bcd(T.S2));
