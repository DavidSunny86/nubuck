...
public:
 ...
 struct MeshInf {
     ...
     rat_vector localPos[4];
     rat_vector center;
 } meshInf;

 // returns the face of the mesh that is opposite to vertex vertIdx
 face GetMeshFace(int vertIdx) const;

 int FindLinkVertex(const Simplex* const other) {
     // incidentally this returns 5 - get_link_index() and I'm suspecting
     // this is a bug in get_link_index() but the value isn't actually used
     // anyways

     if(bcd == other) return 1;
     if(acd == other) return 2;
     if(abd == other) return 3;
     if(abc == other) return 4;
...
be neighbours, if they have 3 vertices together, or if they have 2 vertices together and both are hulltriangles.
 ...
 public:
     // cj --
     GRAPH<d3_rat_point, int>* pmesh;

     void                       SetMesh(GRAPH<d3_rat_point, int>* pmesh) { this->pmesh = pmesh; }
     GRAPH<d3_rat_point, int>&  GetMesh() { return *pmesh; }
